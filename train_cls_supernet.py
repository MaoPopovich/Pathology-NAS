...
from data.dataset import LMDBDataset, TinyImagenet, init_imagenet
...
parser.add_argument("--eval_classes", type=int, default=1000)
...
def train(
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if len(input) == 2: # For classification task, data augmentation is dismissed. we only use first image of pathology datasets
            input = input[0]
...
def main():
    ...
    if args.use_lmdb:
        train_dataset = LMDBDataset(
            ...
            lmdb_path=f"./dataset/lmdb_imgs_{args.img_size}",
        )
    else:
        ...
        #     pretrained=False
        # )
        train_dataset, val_dataset = init_imagenet(
            data_dir = '/home/qinghua_mao/work/ImageNet_1k/train',
            pretrained = True,
            args=args
        )
    ...
    if args.model == 'cnn':
        ...
        depth_list = [12,13,14]
        depth_idx = np.random.randint(3)
        attn_idx = random_choice(num_choice=4, layers=depth_list[depth_idx])
        mlp_idx = random_choice(num_choice=3, layers=depth_list[depth_idx])
    ...
    if args.eval_only:
        val_loss, val_acc = validate(val_loader, model_without_ddp)
        print(
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            _, _ = train(train_loader, model, criterion, optimizer, scaler, epoch, mixup_fn)
