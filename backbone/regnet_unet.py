...
class RegUnet(nn.Module):
    setting_dict = {
        "RegNetY16gf": [32, 224, 448, 1232, 3024],
        "RegNetY32gf": [32, 232, 696, 1392, 3712],
        "RegNetY128gf": [32, 528, 1056, 2904, 7392],
    }
    ...
    ckpt_dict = {
        "RegNetY16gf": "regnety_16gf.torch",
        "RegNetY32gf": "regnety_32gf.torch",
        "RegNetY128gf": "regnety_128gf.torch",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        net: RegNet = self.arch_dict[regnet_name]()

        if use_pretrain:
            ckpt = torch.load(
                f"/home/inspur/.cache/swag/{self.ckpt_dict[regnet_name]}",
                map_location="cpu",
            )
        ...
        self.encoder = create_feature_extractor(
            ...
            return_nodes={
                "stem.stem.2": "0",
                "trunk_output.block1": "1",
                "trunk_output.block2": "2",
                "trunk_output.block3": "3",
                "trunk_output.block4": "4",
            },
        )
        ...
        nodes_dim = {
            "0": self.setting_dict[regnet_name][0],
            "1": self.setting_dict[regnet_name][1],
            "2": self.setting_dict[regnet_name][2],
            "3": self.setting_dict[regnet_name][3],
            "4": self.setting_dict[regnet_name][4],
        }
...
def regnety16gf_unet(num_classes=7, freeze_encoder=False, **kwargs):
    weights = kwargs.pop('weights')
    return RegUnet(
        regnet_name="RegNetY16gf",
        num_classes=num_classes,
        freeze_encoder=freeze_encoder,
        use_pretrain=weights is not None,
        **kwargs,
    )


def regnety32gf_unet(num_classes=7, freeze_encoder=False, **kwargs):
    weights = kwargs.pop('weights')
    return RegUnet(
        regnet_name="RegNetY32gf",
        num_classes=num_classes,
        freeze_encoder=freeze_encoder,
        use_pretrain=weights is not None,
        **kwargs,
...
def regnety128gf_unet(num_classes=7, freeze_encoder=False, **kwargs):
    weights = kwargs.pop('weights')
    return RegUnet(
        regnet_name="RegNetY128gf",
        num_classes=num_classes,
        freeze_encoder=freeze_encoder,
        use_pretrain=weights is not None,
        **kwargs,
